...
export function compareTimer(cur, prev) {
  const steps = [];
  cur.timer.steps.forEach((s, i) => {
    const p = prev.timer.steps[i];
  ...
  const totalDelta = cur.timer.totalSec - prev.timer.totalSec;
...
export function sideBySide(cur, prev) {
  ...
  const n = Math.max(cur.timer.steps.length, prev.timer.steps.length);
  for (let i = 0; i < n; i++) {
    const s = cur.timer.steps[i];
    const p = prev.timer.steps[i];
  ...
  const total = row('타이머', '총 시간', formatSec(cur.timer.totalSec), formatSec(prev.timer.totalSec), significant(cur.timer.totalSec - prev.timer.totalSec));
  if (total.differs) total.note = formatDelta(cur.timer.totalSec - prev.timer.totalSec);
  rows.push(total);
  ...
  for (const it of SURVEY_ITEMS) {
    rows.push(srow(it.label, (b) => (sv(b) ? [level(INTENSITY_WORDS, sv(b).items[it.key].level), ...(sv(b).items[it.key].kinds ?? [])].join(' · ') : '설문 전')));
  }
  rows.push(srow(WORDS.offFlavor.label, (b) => (sv(b) ? (sv(b).offFlavors.length ? sv(b).offFlavors.join(', ') : '없음') : '설문 전')));
  rows.push(srow('만족도', (b) => (sv(b) ? level(LIKING_WORDS, sv(b).liking) : '설문 전')));
  return rows;
}
