...
export const EVENTS = {
  ...
  'bean.save': '원두 저장 — data: { beanId, name, nameAuto(이름 자동 조합), status(active|consumed), totalG(구매 무게 g), remainingG(남은 원두 추정 g) }',
  ...
  'share.build': 'AI 공유 묶음 만들기 — data: { format, scope(with|single|selected), brews(담은 건수), previous(bool), sameRecipeAndBean(bool) }',
  ...
  'aiAdvice.import': 'AI 제안(JSON) 기록에 저장 — data: { changes(바꾸는 것 수), next(값이 있는 칸), warnings, confidence, via(paste|file) }',
  'aiAdvice.importFail': 'AI 제안 가져오기 실패 — data: { stage(parse|validate), errors(앞 5개), via }',
  'aiAdvice.delete': 'AI 제안 지움 — data: { importedAt }',
  'aiAdvice.promptCopy': '결과 받기 프롬프트 복사 — data: { ok(클립보드에 들어갔나) }',
  'export': '내보내기 — data: { brews, beans, grinders, servers, recipes, logs(포함했으면 줄 수) }',
