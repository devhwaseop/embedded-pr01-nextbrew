...
export const EVENTS = {
  ...
  'brew.start': '추출 시작(타이머 화면의 [시작]) — data: { recipeId, doseG, hotWaterG, iceG(넣은 얼음), iceTargetG(추천 얼음), dripper, ratio(원두와 물의 비율 1:x 의 x), customRatio(레시피 비율과 다르게 정했으면 true), readySec(준비 화면에서 넘어온 뒤 [시작]까지 초) }',
  ...
  'brew.pressAbort': '버튼을 누른 채 밖으로 밀어 실행하지 않음 — data: { button(start|next|end), stepIndex }',
