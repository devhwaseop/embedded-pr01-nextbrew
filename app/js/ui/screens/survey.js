...
import { h, section, chips, tagEditor, modal, termOf } from '../dom.js';
...
export function surveyScreen(id) {
  ...
  async function onSave() {
    const k = await modal({
      ...
      actions: [
        { key: 'save', label: '저장만' },
        { key: 'share', label: '저장 후 공유', primary: true },
      ],
    });
    if (!k) return;
    ...
    logEvent('survey.save', { answered: levels.filter((v) => v != null).length, skipped: levels.filter((v) => v == null).length, share: k === 'share' }, { brewId: b.id });
    location.hash = k === 'share' ? `#/brew/${b.id}/share` : `#/brew/${b.id}`;
  }
  ...
  return h(
    ...
    h(
      ...
      h('button', { type: 'button', onClick: () => detail.classList.toggle('hidden') }, '추출 보기'),
    ),
    detail,
    ...
    extra,
    h('button', { class: 'primary big wide', onClick: onSave }, '저장 및 공유'),
  );
}
